...
function kento_pvc_top_geo()
	{	
		...
		$date = $_POST['kpvc_date_geo'];
		if(isset($date))
			{
				$result = $wpdb->get_results("SELECT country FROM $table GROUP BY country ORDER BY COUNT(country) DESC LIMIT 10", ARRAY_A);
				$total_rows = $wpdb->num_rows;
			}
		else
			{
		...
		$referer.= "<thead><tr>";
		$referer.= "<th scope='col' class='manage-column column-name' ><strong>Rank</strong></th>";
		$referer.= "<th scope='col' class='manage-column column-name' ><strong>Country</strong></th>";
		$referer.= "</tr></thead>";
		$referer.= "<tfoot><tr>";
		$referer.= "<th scope='col' class='manage-column column-name' ><strong>Rank</strong></th>";
		$referer.= "<th scope='col' class='manage-column column-name' ><strong>Country</strong></th>";		
		...
		while($total_rows>$i)
			{	
				if($result[$i]['country']=="none" || $result[$i]['country']==NULL  )
					{
